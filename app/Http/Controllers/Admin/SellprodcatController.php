...
class SellprodcatController extends Controller
{
    //
    public function index(){ 
        $data['page_title'] = "Product Category";
        $productcats = Sellprodcat::paginate('100');
        $data['productcats'] = $productcats;
        return view('admin.sellprodcat.list')->with($data);
    }

    public function addCat(){ 
        $data['page_title'] = "Add Product Category";
        return view('admin.sellprodcat.add')->with($data);
    }
    ...
    public function editCat(Request $request){ 
        ...
        $cat_detail = Sellprodcat::where('id',$request->cat_id)->first();
        $data['cat_detail'] = $cat_detail;
        return view('admin.sellprodcat.edit')->with($data);
    }
    ...
    public function updateCat(Request $request){
        ...
        $Productcat = Sellprodcat::find($request->cat_id);
        //On left field name in DB and on right field name in Form/view
        if($Productcat) {
        $Productcat->cat_name = $request->input('cat_name');
        $Productcat->cat_slug = $request->input('cat_slug');
    ...
    public function deleteCat(Request $request){
        Sellprodcat::where('id', $request->cat_id)->delete();
